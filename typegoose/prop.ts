...
import { models, schema, virtuals } from './data';
...
const baseProp = (rawOptions, Type, target, key, isArray = false) => {
  ...
  if (ref) {
    schema[name][key] = {
      ...schema[name][key],
      type: (rawOptions.idType?rawOptions.idType:mongoose.Schema.Types.ObjectId),
      ref: ref.name,
    };
    return;
  }
  ...
  const subSchemaObject = (models[instance.constructor.name]) ?
                  models[instance.constructor.name].schema : schema[instance.constructor.name];
  ...
  if (isArray) {
    if (subSchemaObject) {
      schema[name][key][0] = subSchemaObject;
    } else {
      schema[name][key][0] = {
        ...schema[name][key][0],
        ...options,
